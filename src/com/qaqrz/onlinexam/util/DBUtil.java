...
public class DBUtil {
	...
	/**
	 * 该语句必须是SQL INSERT,UPDATE,DELETE语句
	 * 
	 * @param sql
	 * @return
	 * @throws Exception
	...
	/**
	 * 该语句必须是SQL INSERT,UPDATE,DELETE语句:insert into table values(?,?,?,?)
	 * 
	 * @param sql
	...
	public Object getObject(Class<?> type, String sql, Object[] paramList) throws Exception {
		...
		Map<String, Object> map = getObject(sql, paramList);
		if (map != null) {
			for (int i = 0; i < propertyDescriptors.length; i++) {
				...
				if (map != null && map.containsKey(propertyName)) {
					...
					try {
						...
						logger.info("检查一下table列,和实体类属性:" + propertyName + "是否一致,并且是否是" + value.getClass() + "类型");
						throw new Exception(
								"检查一下table列,和实体类属性:" + propertyName + "是否一致,并且是否是" + value.getClass() + "类型");
					}
				}
			}
		} else {
	...
	public Page getQueryPage(Class<?> type, String sql, Object[] params, Page page) {
		...
		try {
			list = this.getQueryList(type,
					...
					params);
			countMap = this.getObject(" " + "select count(*) c from (" + sql + ") as t ");
	...
	public Map<String, Object> getObject(String sql, Object[] paramList) throws Exception {
		...
		try {
			...
			rs = getResultSet(pstmt);
			List<Map<String, Object>> list = getQueryList(rs);
			if (list.isEmpty()) {
				return null;
			}
